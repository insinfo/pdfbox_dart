...
import '../../crypto/asn1/core/legacy_ber.dart';
import '../../crypto/asn1/core/legacy_der.dart';
import '../../crypto/cryptography/buffered_block_padding_base.dart';
...
import '../../crypto/cryptography/message_digest_utils.dart';
import '../../crypto/cryptography/pkcs1_encoding.dart';
import '../../crypto/cryptography/rsa_algorithm.dart';
import '../../crypto/cryptography/signature_utilities.dart';
        ...
        if (type.id == PkcsObjectId.data.id) {
          ...
          seq.objects!.forEach((dynamic subSequence) {
            ...
            if (subSequenceCollection.id!.id == PkcsObjectId.certBag.id) {
              ...
              final String name = _bytesToHex(localId!.getOctets()!);
              if (key == null) {
                _keys.setValue(name, keyEntry);
              } else {
                _localIdentifiers[key] = name;
    ...
    certificateChain.forEach((Asn1SequenceCollection asn1Collection) {
      ...
      if (isUnmarkedKey) {
        if (_keyCertificates.isEmpty) {
          final String name = _bytesToHex(certId.id!);
...
class _Pkcs12AlgorithmGenerator extends _PasswordGenerator {
  ...
  List<int> generateDerivedKey(int? id, int length) {
    ...
    for (int i = 1; i <= c; i++) {
      ...
      builder.add(d);
      builder.add(tempBytes);
      a = sha1.convert(builder.takeBytes()).bytes;
...
List<int> _encodeBigEndian(String value) {
  final List<int> out = List<int>.filled(value.length * 2, 0);
