...
/// Handler to generate the strikeout annotations appearance.
class PDStrikeoutAppearanceHandler extends PDAbstractAppearanceHandler {
  PDStrikeoutAppearanceHandler(PDAnnotation annotation, [PDDocument? document])
      : super(annotation, document);

  @override
  void generateNormalAppearance() {
    final annot = getAnnotation();
    if (annot is! PDAnnotationStrikeout) return;
    ...
    try {
      ...
      for (int i = 0; i < nQuads; ++i) {
        ...
        if (len0 != 0) {
          ...
          x0 += (pathsArray[i * 8] - pathsArray[i * 8 + 4]) /
              len0 *
              (len0 / 2 - ab.width);
