...
import '../../codestream/writer/codestream_writer.dart';
...
import 'cblk_rate_dist_stats.dart';
import 'coded_cblk_data_src_enc.dart';
import 'post_comp_rate_allocator.dart';
import 'layers_info.dart';
import 'ebcot_layer.dart';
...
import '../../codestream/writer/bit_output_buffer.dart';
...
class EBCOTRateAllocator extends PostCompRateAllocator {
  ...
  /// </ul>
  late List<List<List<List<List<CBlkRateDistStats>>>>> cblks;
  ...
  /// </ul>
  late List<List<List<List<List<List<int>>>>>> truncIdxs;
  ...
  ///
  /// @see ProgressionType
  EBCOTRateAllocator(CodedCBlkDataSrcEnc src, LayersInfo lyrs,
      CodestreamWriter writer, EncoderSpecs encSpec, ParameterList pl)
    ...
    for (int t = 0; t < nt; t++) {
      // Loop on tiles
      nTiles = src.getNumTilesCoord(nTiles);
      ...
      for (int c = 0; c < nc; c++) {
        ...
        cblks[t][c] = List.generate(mrl, (_) => [], growable: false);

        for (l = 0; l < numLayers; l++) {
          truncIdxs[t][l][c] = List.generate(mrl, (_) => [], growable: false);
        }
        ...
        for (int r = 0; r < mrl; r++) {
          ...
          // domain
          trx0 = (tcx0 / (1 << (mrl - 1 - r))).ceil();
          try0 = (tcy0 / (1 << (mrl - 1 - r))).ceil();
          trx1 = (tcx1 / (1 << (mrl - 1 - r))).ceil();
          try1 = (tcy1 / (1 << (mrl - 1 - r))).ceil();
          ...
          for (l = 0; l < numLayers; l++) {
            truncIdxs[t][l][c][r] =
                List.generate(maxsbi, (_) => [], growable: false);
          }
  ...
  void finalize() {
    if (DO_TIMING) {
      ...
      sb.write("  initialization: ");
      sb.write(initTime);
      sb.write(" ms\n");
      sb.write("  layer building: ");
      sb.write(buildTime);
      sb.write(" ms\n");
      sb.write("  final writing:  ");
      sb.write(writeTime);
      sb.write(" ms");
      FacilityManager.getMsgLogger().printmsg(MsgLogger.info, sb.toString());
    }
    // super.finalize(); // No super.finalize in Dart
  }
  ...
  void initialize() {
    ...
    double np; // The number of pixels divided by the number of bits per byte
    ...
    np = src.getImgWidth() * src.getImgHeight() / 8.0;
    ...
    layers = List.generate(numLayers, (_) => EBCOTLayer());
    ...
    List<Progression> prog1;
    prog1 = (encSpec.pocs.getDefault() as List).cast<Progression>();
    ...
    if (nValidProg == 0) {
      throw Error(); // "Unable to initialize rate allocator: No default progression type has been defined."
    }

    // Tile specific values
    for (int t = 0; t < numTiles; t++) {
      if (encSpec.pocs.isTileSpecified(t)) {
        prog1 = (encSpec.pocs.getTileDef(t) as List).cast<Progression>();
        ...
        if (nValidProg == 0) {
          throw Error(); // "Unable to initialize rate allocator: No default progression type has been defined for tile "+t
        }
      }
    } // End loop on tiles
  ...
  void getAllCodeBlocks() {
    int numComps, numTiles;
    int c, r, t, s, sidx, k;
    SubbandAn subb;
    ...
    for (t = 0; t < numTiles; t++) {
      ...
      for (c = 0; c < numComps; c++) {
        ...
        while ((ccb = src.getNextCodeBlock(c, ccb)) != null) {
          ...
          if (pw != null) {
            nEncCblk++;
            pw.updateProgressWatch(nEncCblk, "");
          }
  ...
  void buildAndWriteLayers() {
    int nPrec = 0;
    int maxBytes, actualBytes;
    double rdThreshold;
    SubbandAn sb;
    BitOutputBuffer? hBuff;
    Uint8List? bBuff;
    List<int> tileLengths; // Length of each tile
    int tmp;
          ...
          for (int r = 0; r < mrl; r++) {
            ...
            for (int p = 0; p < nPrec; p++) {
              ...
              if (pktEnc.isPacketWritable()) {
                tmp = bsWriter.writePacketHead(hBuff.getBuffer(),
                    hBuff.getLength(), true, sopUsed, ephUsed);
                tmp += bsWriter.writePacketBody(
    ...
    pktEnc.reset();
    List<Progression> prog; // Progression(s) in each tile
    int cs, ce, rs, re, lye;

    List<int> mrlc = List.filled(nc, 0);
    for (int t = 0; t < nt; t++) {
      //loop on tiles
      List<List<int>> lys = List.generate(nc, (_) => []);
      for (int c = 0; c < nc; c++) {
        mrlc[c] = src.getAnSubbandTree(t, c).resLvl;
        lys[c] = List.filled(mrlc[c] + 1, 0);
      }
      ...
      prog = (encSpec.pocs.getTileDef(t) as List).cast<Progression>();
      ...
      for (int prg = 0; prg < prog.length; prg++) {
        ...
        switch (prog[prg].type) {
          ...
          case ProgressionType.RES_POS_COMP_LY_PROG:
            ...
            break;
          default:
            throw Error(); // "Unsupported bit stream progression type"
  ...
  void writeResLyCompPos(
      int t, int rs, int re, int cs, int ce, List<List<int>> lys, int lye) {
    bool sopUsed; // Should SOP markers be used ?
    bool ephUsed; // Should EPH markers be used ?
    int nc = src.getNumComps();
    List<int> mrl = List.filled(nc, 0);
    SubbandAn sb;
    double threshold;
    BitOutputBuffer? hBuff;
    Uint8List? bBuff;
      ...
      for (int l = minlys; l < lye; l++) {
        ...
        for (int c = cs; c < ce; c++) {
          ...
          for (int p = 0; p < nPrec; p++) {
            ...
            // set boolean ephUsed here (EPH markers)
            ephUsed = (encSpec.ephs.getTileDef(t) as String) == "on";
  ...
  void writeCompPosResLy(
    ...
    img.Coord nTiles = src.getNumTilesCoord(null);
    img.Coord tileI = src.getTile(null);
        ...
        for (int c = cs; c < ce; c++) {
          ...
          for (int r = rs; r < re; r++) {
            ...
            for (int l = minlys; l < lye; l++) {
              ...
              hBuff = pktEnc.encodePacket(l + 1, c, r, t, cblks[t][c][r],
                  truncIdxs[t][l][c][r], hBuff, bBuff, nextPrec[c][r]);
  ...
  double optimizeBitstreamLayer(
      int layerIdx, double fmaxt, int maxBytes, int prevBytes) {
    ...
    double fmint; // Minimum of the current threshold interval
    double ft; // Current threshold
    SubbandAn sb; // Current subband
    BitOutputBuffer? hBuff; // The packet head buffer
    Uint8List? bBuff; // The packet body buffer
          ...
          for (int r = 0; r < numLvls; r++) {
            ...
            for (int p = 0; p < nPrec; p++) {
              ...
              hBuff = pktEnc.encodePacket(layerIdx + 1, c, r, t, cblks[t][c][r],
                  truncIdxs[t][layerIdx][c][r], hBuff, bBuff, p);

              if (pktEnc.isPacketWritable()) {
                bBuff = pktEnc.getLastBodyBuf();
                actualBytes += bsWriter.writePacketHead(hBuff.getBuffer(),
                    hBuff.getLength(), true, sopUsed, ephUsed);
                actualBytes += bsWriter.writePacketBody(
                    bBuff,
                    pktEnc.getLastBodyLen(),
                    true,
                    pktEnc.isROIinPkt(),
  ...
  void findTruncIndices(int layerIdx, int compIdx, int lvlIdx, int tileIdx,
    ...
    sb = subb;
    while (sb.subbHH != null) {
      sb = sb.subbHH as SubbandAn;
    }
    minsbi = (lvlIdx == 0) ? 0 : 1;
    maxsbi = (lvlIdx == 0) ? 1 : 4;
    ...
    sb = subb.getSubbandByIdx(lvlIdx, minsbi) as SubbandAn;
    for (int s = minsbi; s < maxsbi; s++) {
      //loop on subbands
      yend = prec.cblk[s].length;
