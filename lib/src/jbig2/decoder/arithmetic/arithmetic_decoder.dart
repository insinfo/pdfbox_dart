...
class ArithmeticDecoder {
  ...
  int a = 0;
  int c = 0; // Using int for long, Dart int is 64-bit
  int ct = 0;
  ...
  final SubInputStream iis;

  ArithmeticDecoder(this.iis) {
    init();
  }

  void init() {
    streamPos0 = iis.getStreamPosition();
    b = iis.read();
  ...
  int lpsExchange(CX cx, int icx, int qeValue) {
    final int mps = cx.mps();
