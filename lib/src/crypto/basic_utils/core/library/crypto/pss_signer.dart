...
class LocalPSSSigner implements Signer {
  ...
  LocalPSSSigner(this._cipher, this._contentDigest, this._mgfDigest,
  ...
  PSSSignature generateSignature(Uint8List message) {
    ...
    if (_sLen != 0) {
      ...
      _arrayCopy(_salt, 0, _mDash, _mDash.length - _sLen, _sLen);
    }
    ...
    _arrayCopy(_salt, 0, _block, _block.length - _sLen - _hLen - 1, _sLen);
    ...
    _arrayCopy(h, 0, _block, _block.length - _hLen - 1, _hLen);
  ...
  bool verifySignature(Uint8List message, covariant PSSSignature signature) {
    ...
    _arrayCopy(b, 0, _block, _block.length - b.length, b.length);
    ...
    if (_sSet) {
      ...
      _arrayCopy(_block, _block.length - _sLen - _hLen - 1, _mDash,
  ...
  Uint8List _maskGeneratorFunction1(
    ...
    while (counter < (length ~/ _mgfhLen)) {
      ...
      _arrayCopy(hashBuf, 0, mask, counter * _mgfhLen, _mgfhLen);
      counter++;
    }
    ...
    if ((counter * _mgfhLen) < length) {
      ...
      _arrayCopy(hashBuf, 0, mask, counter * _mgfhLen,
