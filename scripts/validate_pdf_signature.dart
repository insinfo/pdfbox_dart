...
Future<void> main(List<String> args) async {
  if (args.isEmpty) {
    print('Uso: dart scripts/validate_pdf_signature.dart <arquivo.pdf> [--verbose]');
    exit(64);
  }
  ...
  final result = validatePdfSignature(pdfBytes, verbose: verbose);
...
PdfSignatureValidationResult validatePdfSignature(Uint8List pdfBytes,
  ...
  final actualDigest = CryptoUtils.getHash(signedPortion, algorithmName: 'SHA-256');
  final actualDigestBytes = _hexToBytes(actualDigest);
  final digestMatches = _constantTimeEquals(actualDigestBytes, cms.messageDigest ?? Uint8List(0));
  ...
  try {
    ...
    if (pub != null) {
      publicKeySource = 'CMS';
    } else if (File('user.crt').existsSync()) {
      final pem = File('user.crt').readAsStringSync();
    ...
    if (pub != null && cms.signedAttrsEncoded != null && cms.signature != null) {
  ...
  if (verbose) {
    ...
    print('messageDigest (CMS): ${expected != null ? _bytesToHex(expected) : 'n/a'}');
    ...
    if (cms.signedAttrsImplicit != null) {
      print('signedAttrs [0] hex   : ${_bytesToHex(cms.signedAttrsImplicit!)}');
    }
    if (cms.signedAttrsValue != null) {
      print('signedAttrs value hex : ${_bytesToHex(cms.signedAttrsValue!)}');
    }
    if (cms.signedAttrsEncoded != null) {
      print('signedAttrs DER hex   : ${_bytesToHex(cms.signedAttrsEncoded!)}');
    }
    if (cms.signedAttrsEncoded != null) {
      try {
        ...
        if (top.valueBytes != null) {
          ...
          print('signedAttrs inner     : ${inner.runtimeType} tag=0x$innerTag entries=$count');
        }
      } catch (_) {}
    }
  }
...
_CmsParsed _parseCms(Uint8List cmsBytes) {
  ...
  final oi = top.elements!.elementAt(0) as ASN1ObjectIdentifier; // 1.2.840.113549.1.7.2
  ...
  final sdSeq = ASN1Parser(signedDataWrapper.valueBytes).nextObject() as ASN1Sequence;
  ...
  var siIdx = 0;
  siIdx++; // version
  final sid = signerInfo.elements!.elementAt(siIdx++); // issuerAndSerialNumber
  ...
  if (certsObj != null) {
    ...
    for (final el in children) {
      if (el is ASN1Sequence && el.elements != null && el.elements!.length >= 3) {
