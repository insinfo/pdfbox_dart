...
- `HeaderDecoder.readMainHeader` também consome o marcador `COD`, popular `DecoderSpecs` com número de camadas (`nls`), ordem de progressão (`pos`), blocos (`cblks`), flag de transformada de componentes (`cts`) e detecta o uso de precincts/sinalizadores SOP/EPH. O teste `header_decoder_main_header_test.dart` valida os novos campos a partir de um codestream sintético.
...
- `FileBitstreamReaderAgent` agora honra POCs (`decSpec.pcs`), segmentando a varredura dos pacotes por progressão declarada e reutilizando o cache de precintos para todos os modos (LRCP, RLCP, RPCL, PCRL, CPRL); o `HeaderDecoder` passou a parsear os marcadores POC, alimentar o `decSpec.pcs`, registrar `Psot` por tile-part e disponibilizar cabeçalhos empacotados (PPM/PPT) para o `PktDecoder`.
- `HeaderDecoder` passou a interpretar os marcadores QCD/QCC, preenchendo `DecoderSpecs.qts/qsss/gbs` com os estilos de quantização e tabelas de expoente/passo por componente; `HeaderInfo` agora armazena os guard bits e os testes em `test/jj2000/j2k/codestream/header_decoder_main_header_test.dart` cobrem cenários reversíveis (QCD) e overrides expostos via QCC.
- Os marcadores COC também foram portados no `HeaderDecoder`, permitindo overrides por componente das dimensões de code-block, níveis de decomposição e opções de codificação; `DecoderSpecs.cblks/dls/ecopts/pss` recebem as variações apropriadas e novos testes garantem cobertura para esse fluxo.
- `HeaderDecoder.parseTilePartHeader` percorre os marcadores de tile-part (COD/COC/QCD/QCC/POC/PPT), aplica overrides scoped por tile em `DecoderSpecs`, mantém o `RandomAccessIO` reposicionado no `SOD` e registra os metadados correspondentes em `HeaderInfo`; cenário coberto por `test/jj2000/j2k/codestream/reader/header_decoder_test.dart`.
- `HeaderDecoder.parseNextTilePart` agora dirige a leitura sequencial do codestream: localiza o próximo `SOT`, registra a ordem das tiles, delega para `parseTilePartHeader` e valida o `SOD` subsequente. Também preserva o offset pós-`SOD` e o comprimento útil de cada tile-part para alimentar os estágios de leitura de pacotes; os testes em `test/jj2000/j2k/codestream/reader/header_decoder_test.dart` cobrem tanto o fluxo completo com um codestream sintético quanto um caso multi-tile exercitando agregação de `Psot`, offsets e payloads.
...
- Implementado `SaslPrep` (RFC 4013) para normalizar senhas da revisão 6 com NFKC, regras bidi (via pacote `bidi`) e conjunto ampliado de testes unitários cobrindo mapeamentos, proibições e validações de uso.
- Portado o gerador de dicionário de criptografia da revisão 6 (`/U`, `/UE`, `/O`, `/OE`, `/Perms`) com geração de chave AES-256, filtros `/StdCF`/`AESV3`, aplicação de SASLprep nas senhas e verificações automatizadas dos campos produzidos.
...
- `/Encrypt` agora é promovido a objeto indireto pelo `PDDocument` e ambos os caminhos do `COSWriter` (tabelas clássicas e streams) escrevem o trailer com referência indireta, cobrindo também o `SimplePdfWriter`; TODO seguir com `willEncrypt`, criptografia efetiva dos objetos e demais ganchos de segurança do Java.
