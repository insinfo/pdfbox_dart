...
import 'package:pdfbox_dart/src/ucar/jpeg/jj2000/j2k/image/output/img_writer_pgm.dart';

typedef _ComponentSamples = List<int>;

void main() {
  group('ImgWriterPgm', () {
    test('downshifts high bit depth components', () {
      final source = _SingleTileSource(
        ...
        bitDepths: const <int>[10],
        fixedPoints: const <int>[0],
      );

      final tempDir = Directory.systemTemp.createTempSync('img_writer_pgm_');
      addTearDown(() => tempDir.deleteSync(recursive: true));
      final outputPath = '${tempDir.path}/component.pgm';

      final writer = ImgWriterPgm.fromPath(outputPath, source, 0);
      ...
      final output = File(outputPath).readAsBytesSync();
      var headerEnd = 0;
      var newlineCount = 0;
      while (headerEnd < output.length && newlineCount < 3) {
        if (output[headerEnd] == 0x0a) {
          newlineCount++;
        }
        headerEnd++;
      }
      final header = ascii.decode(output.sublist(0, headerEnd));
      expect(header, equals('P5\n3 1\n255\n'));

      final raster = output.sublist(headerEnd);
      expect(raster, equals(<int>[128, 255, 255]));
    });
  });
}
